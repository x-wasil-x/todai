...
const pickGasPayerHeaders = ({ headers }) => ({
  headers: pick(headers, ['x-gas-payer-sig', 'x-gas-payer-pk']),
});

const verifyAssetClaim = async ({
  claim,
  challenge,
  trustedIssuers = [],
  trustedParents = [],
  ...
  const { state } = await client.getAssetState(
    { address },
  ...
  if (state.owner !== ownerDid) {
    throw new Error(
      'Invalid asset claim: owner does not match with on chain state',
    );
  }
  if (
    trustedIssuers.length &&
    trustedIssuers.includes(state.issuer) === false
  ) {
    throw new Error('Invalid asset claim: asset issuer not in whitelist');
  }
  if (
    trustedParents.length &&
    trustedParents.includes(state.parent) === false
  ) {
