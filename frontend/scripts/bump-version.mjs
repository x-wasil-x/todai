...
async function main() {
  execSync('bumpp --no-tag --no-commit --no-push package.json', {
    stdio: 'inherit',
  });
  ...
  console.log(
    chalk.greenBright(`[info]: start to modify blocklet version to ${version}`),
  );
  ...
  try {
    ...
    console.error(
      chalk.redBright(
        'Could not get git log, please write changelog manually.',
      ),
    );
  }
